...
function getBaseStationId(stationId) {
  ...
  do {
    ...
    withoutSuffix = withoutSuffix.replace(
      /_(top|base|mid|deep|surface|below_ground)$/i,
      "",
    );
  ...
  withoutSuffix = withoutSuffix.replace(
    /_(piccadilly|district|circle|bakerloo|central|northern|victoria|jubilee|metropolitan|elizabeth|dlr|overground|hnc)$/i,
    "",
  );

  // Remove other common suffixes
  withoutSuffix = withoutSuffix.replace(
    /_(nr|cr|hub|osi|validator|zone|tube|rail)$/i,
    "",
  );

  // Remove special suffixes
  withoutSuffix = withoutSuffix.replace(
    /_(special|divider|shared|isi|tunnel)$/i,
    "",
  );
...
function matchStation(tflStation, ourStationIds) {
  ...
  if (
    (originalName.includes("king") &&
      originalName.includes("cross") &&
      (originalName.includes("pancras") ||
        originalName.includes("st pancras"))) ||
    originalName.includes("kings cross st pancras")
  ) {
    const match = ourStationIds.find((id) => {
      const idLower = id.toLowerCase();
      return (
        idLower.includes("king_cross_st_pancras") ||
        idLower === "kings_cross_st_pancras"
      );
    });
    if (match) {
      console.log(
        `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
      );
      return match;
    }
  }
  ...
  if (originalName.includes("heathrow")) {
    if (
      originalName.includes("terminal 4") ||
      originalName.includes("t4") ||
      originalName.includes("t 4") ||
      originalName.match(/terminal\s*4/i)
    ) {
      // Prefer piccadilly, then elizabeth
      const match =
        ourStationIds.find(
          (id) => id.toLowerCase() === "heathrow_4_piccadilly",
        ) ||
    ...
    if (
      originalName.includes("terminal 5") ||
      originalName.includes("t5") ||
      originalName.includes("t 5") ||
      originalName.match(/terminal\s*5/i)
    ) {
      const match =
        ourStationIds.find(
          (id) => id.toLowerCase() === "heathrow_5_piccadilly",
        ) ||
    ...
    if (
      originalName.includes("terminal 1") ||
      originalName.includes("terminal 2") ||
      originalName.includes("terminal 3") ||
      originalName.includes("terminals 1-2-3") ||
      originalName.includes("terminals 1 2 3") ||
      originalName.match(/terminal\s*[123]/i)
    ) {
      const match = ourStationIds.find(
        (id) => id.toLowerCase() === "heathrow_23",
      );
      if (match) {
        console.log(
          `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  if (originalName.includes("shepherd") && originalName.includes("bush")) {
    if (originalName.includes("central") || originalName.includes("tube")) {
      const match = ourStationIds.find(
        (id) => id.toLowerCase() === "shepherds_bush_tube",
      );
      if (match) {
        console.log(
          `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  if (originalName.includes("euston")) {
    if (originalName.includes("overground") || originalName.includes("rail")) {
      const match = ourStationIds.find(
        (id) => id.toLowerCase() === "euston_overground_nr",
      );
      if (match) {
        console.log(
          `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
    ...
    if (
      originalName.includes("northern") ||
      originalName.includes("victoria")
    ) {
      const match = ourStationIds.find(
        (id) => id.toLowerCase() === "euston_northern_victoria",
      );
      if (match) {
        console.log(
          `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  if (originalName.includes("st james") && originalName.includes("street")) {
    const match = ourStationIds.find(
      (id) => id.toLowerCase() === "st_james_street",
    );
    if (match) {
      console.log(
        `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  if (originalName.includes("cutty sark")) {
    const match = ourStationIds.find((id) =>
      id.toLowerCase().includes("cutty_sark"),
    );
    if (match) {
      console.log(
        `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  if (originalName.includes("liverpool") && originalName.includes("street")) {
    ...
    const match = ourStationIds.find((id) => {
      const idLower = id.toLowerCase();
      return (
        idLower.includes("liverpool_street") &&
        (idLower.includes("circle") || idLower.includes("elizabeth"))
      );
    });
    if (match) {
      console.log(
  ...
  if (originalName.includes("paddington")) {
    if (originalName.includes("deep") || originalName.includes("elizabeth")) {
      ...
      const match = ourStationIds.find((id) => {
        const idLower = id.toLowerCase();
        return (
          (idLower.includes("paddington_deep") ||
            idLower.includes("paddington_elizabeth")) &&
          (idLower.includes("base") ||
            idLower.includes("deep") ||
            idLower.includes("elizabeth"))
        );
      });
      if (match) {
        console.log(
  ...
  if (originalName.includes("moorgate")) {
    const match = ourStationIds.find((id) =>
      id.toLowerCase().includes("moorgate"),
    );
    if (match) {
      console.log(
        `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  if (originalName.includes("eastcote")) {
    if (originalName.includes("piccadilly")) {
      const match = ourStationIds.find(
        (id) => id.toLowerCase() === "eastcote_piccadilly",
      );
      if (match) {
        console.log(
          `  Special match: "${tflStation.commonName || tflStation.name}" -> ${match}`,
  ...
  const tflNormalized = normalizeStationName(
    tflStation.commonName || tflStation.name,
  );
...
async function fetchStationsForMode(mode, useCache, cacheMaxAge) {
  ...
  if (useCache && fs.existsSync(cacheFile)) {
    try {
      ...
      if (age < cacheMaxAge) {
        console.log(
          `Using cached ${mode} stations (${Math.round(age / (60 * 60 * 1000))}h old)`,
        );
        const cachedData = JSON.parse(fs.readFileSync(cacheFile, "utf8"));
        const stations =
          cachedData.stopPoints ||
        ...
        console.log(
          `Cache expired for ${mode} (${Math.round(age / (24 * 60 * 60 * 1000))}d old), fetching fresh data...`,
        );
      }
    } catch (error) {
      console.warn(
        `Error reading cache for ${mode}, fetching fresh data:`,
        error.message,
      );
    }
  }
...
async function main() {
  ...
  for (const mode of MODES) {
    const stations = await fetchStationsForMode(
      mode,
      !forceRefresh,
      cacheMaxAge,
    );
  ...
  for (const tflStation of allTflStations) {
    ...
    const isMainStation =
      tflStation.stopType &&
      tflStation.stopType.includes("Station") &&
  ...
  fs.writeFileSync(
    path.join(dataDir, "tfl-station-map.json"),
    JSON.stringify(tflStationMapObj, null, 2),
  );
  ...
  const initialUnmatched = ourStationIds.filter(
    (id) => !matchedStations.has(id),
  );
  ...
  while (iterations < maxIterations) {
    const currentUnmatched = ourStationIds.filter(
      (id) => !matchedStations.has(id),
    );
    ...
    const newUnmatchedCount = ourStationIds.filter(
      (id) => !matchedStations.has(id),
    ).length;
    if (
      newUnmatchedCount === 0 ||
      newUnmatchedCount === previousUnmatchedCount
    ) {
  ...
  const manualCoordinates = {
    ...
    Old_Oak_Common: {
      lat: 51.5194,
      lng: -0.2319,
      name: "Old Oak Common (under construction)",
    },
    South_Greenford: {
      lat: 51.542424,
      lng: -0.34605,
    ...
    Surrey_Canal: {
      lat: 51.493196,
      lng: -0.047519,
      name: "Surrey Canal (likely Surrey Quays)",
    },
  };
  ...
  fs.writeFileSync(
    path.join(dataDir, "matched-stations.json"),
    JSON.stringify(matchedStationsObj, null, 2),
  );
  fs.writeFileSync(
    path.join(dataDir, "unmatched-stations.json"),
    JSON.stringify(unmatchedTflStations, null, 2),
  );
  ...
  for (const [stationId, geo] of sortedEntries) {
    lines.push(
      `  "${stationId}": { lat: ${geo.lat}, lng: ${geo.lng} }, // ${geo.name}`,
    );
  }
  ...
  // Show coverage
  const coverage = (
    (matchedStations.size / ourStationIds.length) *
    100
  ).toFixed(1);
  console.log(
